...
export function ResultsPanel({ result }: ResultsPanelProps) {
  ...
  const issues = result.final_decision.issues ?? [];
    ...
    <div className="rounded-2xl border border-white/10 bg-white/5 p-6 shadow-lg shadow-black/20">
      ...
      <div className="grid gap-6 xl:grid-cols-2">
        <section className="rounded-xl border border-white/10 bg-slate-950/40 p-4">
          <h3 className="text-sm font-semibold uppercase tracking-wide text-slate-200">Text Preview</h3>
        ...
        <section className="rounded-xl border border-white/10 bg-slate-950/40 p-4">
          <h3 className="text-sm font-semibold uppercase tracking-wide text-slate-200">Final Decision</h3>
          <div className="mt-3 rounded-lg border border-emerald-500/20 bg-emerald-500/10 px-4 py-3">
            <p className="text-sm font-medium text-emerald-200">{result.final_decision.decision}</p>
          </div>

          <div className="mt-4">
            <h4 className="text-xs font-semibold uppercase tracking-wide text-slate-400">Issues</h4>
            {issues.length === 0 ? (
              <p className="mt-2 text-sm text-slate-400">No issues reported by the decision stage.</p>
            ) : (
              <ul className="mt-3 space-y-3">
                {issues.map((issue, index) => (
                  ...
                  >
                    <p className="text-sm font-medium text-amber-200">{issue.agent}</p>
                    <p className="mt-1 text-xs uppercase tracking-wide text-amber-100/80">
                      {issue.status}
                    </p>
                    <p className="mt-2 text-sm text-slate-200">{issue.reason}</p>
                  </li>
                ))}
              </ul>
            )}
      ...
      <section className="mt-6 rounded-xl border border-white/10 bg-slate-950/40 p-4">
        <h3 className="text-sm font-semibold uppercase tracking-wide text-slate-200">Agent Results</h3>
          ...
          {result.agent_results.map((agentResult, index) => (
            ...
            >
              <div className="flex flex-wrap items-center justify-between gap-2">
                <p className="text-sm font-semibold text-white">{agentResult.agent}</p>
              ...
              <p className="mt-2 text-sm text-slate-300">{agentResult.reason}</p>
            </div>
          ))}
        </div>
      </section>
      ...
      <section className="mt-6 rounded-xl border border-white/10 bg-slate-950/40 p-4">
        <div className="flex items-center justify-between gap-3">
          <h3 className="text-sm font-semibold uppercase tracking-wide text-slate-200">Similar Invoices</h3>
          <span className="text-xs text-slate-400">{similarInvoices.length} matches</span>
        </div>

        {similarInvoices.length === 0 ? (
          <p className="mt-3 text-sm text-slate-400">No similar invoices were returned.</p>
        ) : (
          <div className="mt-4 grid gap-3">
            {similarInvoices.map((invoice, index) => (
              <div
                className="rounded-xl border border-white/10 bg-slate-900/60 px-4 py-3"
                key={`${invoice.score}-${index}`}
              >
                <div className="flex items-center justify-between gap-3">
                  <p className="text-sm font-medium text-white">
                    Match Score: {(invoice.score * 100).toFixed(1)}%
                  </p>
                  {invoice.payload.decision ? (
                    <span className="rounded-full border border-cyan-400/20 bg-cyan-400/10 px-2.5 py-1 text-xs uppercase tracking-wide text-cyan-200">
                      {invoice.payload.decision}
                    </span>
                  ) : null}
                </div>
                <p className="mt-2 text-sm text-slate-300">
                  {invoice.payload.text_preview || "No preview available for this related invoice."}
                </p>
              </div>
            ))}
          </div>
